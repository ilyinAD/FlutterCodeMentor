...
func (r *submissionRepository) GetByTaskID(ctx context.Context, taskID int) ([]*domain.Submission, error) {
	query := `
		SELECT id, student_id, task_id, code, github_url, submitted_at, score, status, submission_type
		FROM submissions
		WHERE task_id = $1
...
func (r *submissionRepository) UpdateScore(ctx context.Context, id int, score float64) error {
	query := `UPDATE submissions SET score = $1 WHERE id = $2`
	if _, err := r.pool.Exec(ctx, query, score, id); err != nil {
		return fmt.Errorf("failed to update submission score: %w", err)
	}
	return nil
}
